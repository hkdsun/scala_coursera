...
object Huffman {
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def iter(subtree: CodeTree, subbits: List[Bit], chars: List[Char]) : List[Char] = subtree match{
      case Leaf(ch,_) => iter(tree,subbits,chars :+ ch)
      case Fork(ltree,rtree,_,_) =>
        if(subbits.isEmpty) chars
        else if(subbits.head == 0) iter(ltree,subbits.tail,chars)
        else iter(rtree,subbits.tail,chars)
    }
    iter(tree,bits,List())
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def iterate(branch: CodeTree, chars: List[Char], code: List[Bit], acc: List[Bit]) : List[Bit] =
      if(chars.isEmpty) acc
      else branch match {
      case Leaf(c,_) =>{
        //remember to reverse final value
        if(c == chars.head) iterate(tree,chars.tail,List(),acc ::: code.reverse)
        else List()
      }
      case Fork(l,r,c,_) => iterate(l,chars,0::code,acc) ::: iterate(r,chars,1::code,acc)
    }

    iterate(tree,text,List(),List())
  }
  ...
  def convert(tree: CodeTree): CodeTable = tree match {
    case Leaf(_,_) => List()
    case Fork(l,r,c,w) => {
      val lc = chars(l) map(f => (f,List(0)))
      val rc = chars(r) map(f => (f,List(1)))
      mergeCodeTables(lc ::: convert(l),rc ::: convert(r))
    }
  }
