...
object Huffman {
  ...
  val sampleTree = makeCodeTree(Leaf('a',8),
      makeCodeTree(
        makeCodeTree(Leaf('b',3),makeCodeTree(Leaf('c',1),Leaf('d',1))),
        makeCodeTree(makeCodeTree(Leaf('e',1),Leaf('f',1)),makeCodeTree(Leaf('g',1),Leaf('h',1)))
      ))

  val sampleTree2 =createCodeTree(string2Chars("abbbacdefghaaaaaa"))

  val sampleCode = List(1,0,0,0,1,0,1,0)

  def decodedSample = decode(sampleTree,sampleCode)
  def decodedSample2 = decode(sampleTree2,sampleCode)
  /**
